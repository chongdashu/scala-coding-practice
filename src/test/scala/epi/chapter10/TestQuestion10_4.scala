...
class TestQuestion10_4 extends TestCase {
    ...
    def testSimpleTreeGetLCA_UsingParentLinks = {
        val a = simpleTree.getNodeWithValue("A");
        val b = simpleTree.getNodeWithValue("B");
        val c = simpleTree.getNodeWithValue("C");
        
        val expected = a;
        val actual = a.getLCAUsingParentLinks(b, c);
        
        assertNotNull("LCA(b,c) should be a, which should not be null", actual);
        assertEquals(s"LCA(b,c) = ${expected.value} and not ${actual.value}.", expected, actual)
    }
    
    def testExampleTreeGetLCA_DepthDifferenceOne_UsingParentLinks = {
        ...
        val expected = k;
        val actual = exampleTree.getLCAUsingParentLinks(m, n);
        
        assertNotNull("LCA(m, n) should not be null", actual);
        assertEquals("LCA(m, n) should be k", expected, actual);
    }
    
    def testExampleTreeGetLCA_DepthDifferenceTwo_WithParentLinks = {
        val m = exampleTree.getNodeWithValue(641);
        ...
        val expected = i;
        val actual = exampleTree.getLCAUsingParentLinks(m, p);
